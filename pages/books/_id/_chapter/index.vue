	...
	export default {
		...
		async fetch() {
			...
			const res3 = await fetch(process.env.baseUrl + '/links/' + this.$route.params.id + '/' + this.$route.params.id + '-' + this.$route.params.chapter + '.json')
				...
				const array = ['','Быт','Исх','Лев','Чис','Втор','Нав','Суд','Руфь','1Цар','2Цар','3Цар','2Цар','1Пар','2Пар','Езд','Неем','Есф','Иов','Пс','Прит','Еккл','Песн','Ис','Иер','Плач','Иез','Дан','Ос','Иоил','Ам','Авд','Ион','Мих','Наум','Авв','Соф','Агг','Зах','Мал','Мф','Мк','Лк','Ин','Деян','Иак','1Петр','2Петр','1Ин','2Ин','3Ин','Иуд','Рим','1Кор','2Кор','Гал','Еф','Фил','Кол','1Фес','2Фес','1Тим','2Тим','Тит','Флм','Евр','Откр'];
			...
			getVerseLink: function (e) {
				fetch('/books/' + e.target.dataset.book + '/' + e.target.dataset.book + '-' + e.target.dataset.chap + '.json')
                    ...
                    .then(data => {
						...
						verse.innerHTML = data.verses[ e.target.dataset.vers - 1].verse;

						alert(verse.innerText);  
